#!/usr/bin/env python3
"""
2D Electric Field Solver for Electrospray Thrusters
====================================================
Finite Element Method (FEM) solver for electrostatic fields in axisymmetric geometry
Solves Laplace's equation: ∇²φ = 0 (in vacuum)
Computes electric field: E = -∇φ

Physical System:
- Taylor cone emitter at ground potential (φ = 0)
- Extraction electrode at high voltage (φ = V_ext)
- Axisymmetric geometry (r, z coordinates)
...
# Now we need to extend the solver to 3D, we will first start with square configuration, there are three variables we can control, First number of the needles, Second and third are the spacing between the needles , both the X and Y coordinate of the needles. The extractor is placed at some distance from the tip and only has the negative charge and the needles only have the positive charge

# The 3D solver will be implemented in a new class or function, building upon the principles established in this 2D solver.
# This will involve adapting the finite difference scheme to 3D Cartesian coordinates and handling the more complex boundary conditions for multiple needles and an extractor grid.
# We will also need to consider the computational cost and potential use of iterative solvers for larger 3D grids.
# The 3D solver will likely require a more sophisticated mesh generation approach to accurately represent the complex geometry of multiple needles and the extractor.
# This will be a significant undertaking, requiring careful planning and modular design.
# The next step is to define the basic structure for the 3D solver, starting with a new class.
